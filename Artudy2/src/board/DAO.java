...
public class DAO {
	...
	public ArrayList<VO> getMemberList() {
		...
		try {
			sql = "SELECT NUM, name, TITLE, TIME, HIT, INDENT from board1 order by num asc,ref asc";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public void insertWrite(VO vo, int max) {
		...
		try {
			sql = "INSERT INTO board1(num,name,PASSWORD,TITLE,MEMO,REF) VALUES(SEQ_R.nextval,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
	...
	public VO getView(int idx) {
		...
		try {
			sql = "SELECT name, TITLE, MEMO, TIME, HIT, PASSWORD, REF, INDENT, STEP FROM board1 WHERE NUM=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
	...
	public void insertReply(VO vo, int ref, int indent, int step) {
		...
		try {
			sql = "INSERT INTO board1(num,name, PASSWORD, TITLE, MEMO, REF, INDENT, STEP) "+
					"VALUES(SEQ_R.NEXTVAL,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
